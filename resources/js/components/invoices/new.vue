...
import { routerKey } from "vue-router";
 ...
 const onSave = ()=>{
    if (listCart.value.length >= 1)
    {
        ...
        formData.append('sub_total', form.value.subTotal)
        formData.append('total', form.value.total)
        formData.append('terms and conditions', form.value.terms_and_conditions)
        ...
        <div class="modal__content">
            ...
            <ul style="list-style: none;">
                <li v-for="(item, i) in listProducts" :key="item.id" style="display:grid; grid-template-columns: 30px 350px 15px; align-items: center; padding-bottom: 5px;">
                    ...
                    <button @click="addCart(item)"   style="border: 1px solid #e0e0e0; width:35px; height:35px;">
                        +
                    </button>
                </li>
            </ul>
